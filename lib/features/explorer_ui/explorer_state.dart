...
class ExplorerNotifier extends StateNotifier<ExplorerState> {
  ...
  // hasCutPaths (versi lama, single-mode global buat SATU batch)
  // DIHAPUS — clipboard sekarang per-item, bisa campur Copy+Move
  // sekaligus, jadi tidak ada lagi "satu mode buat semua". Tidak ada
  ...
  /// (lihat FileClipboardNotifier.add di core/clipboard/file_clipboard.
  /// dart) — item lama yang belum di-paste TIDAK hilang, beda dari
  /// versi lama yang mengganti seluruh isi clipboard.
  void copySelected() {
    _fileClipboardNotifier.add(state.selectedPaths.toList(), isCut: false);
    _zipClipboardNotifier.clear(); // cuma 1 JENIS clipboard aktif (file vs zip) dalam satu waktu
    state = state.copyWith(selectedPaths: {});
  }
  ...
  /// panel (_ClipboardPanel), bukan implisit seluruh isi clipboard —
  /// beda dari versi lama, karena sekarang paste bisa parsial (bisa
  /// pilih sebagian item buat ditempel, sisanya nunggu tujuan lain).
  ...
  /// pemanggil (explorer_screen.dart _handleFilePasteSelected)
  /// berdasarkan mode per-item — dijalankan sebagai DUA task terpisah
  /// kalau campuran, karena TaskQueue.copy/move masing-masing cuma
  /// terima satu mode per panggilan.
  ///
  /// TIDAK clear clipboard manual di sini — item yang berhasil
  /// diantar hilang OTOMATIS lewat listener FileCopied/FileMoved di
  /// FileClipboardNotifier begitu task-nya SUKSES (lihat
  /// core/clipboard/file_clipboard.dart). Kalau task gagal, event itu
  /// tidak pernah fire, jadi item otomatis TETAP ADA di clipboard,
  /// siap dicoba paste ulang.
